...
def main(config):
    ...
    cl_uberon_doid_og, x,y = load_ontology.load("0", config)
    efo_cellline_og, x,y = load_ontology.load("10", config)
    cvcl_og,x,y = load_ontology.load("4", config)

    with open(config.ref_path / "term_to_linked_terms.json", "r") as f:
        term_to_linked_terms = json.load(f)    

    term_to_linkedsup_terms = term_to_linked_superterms(cl_uberon_doid_og, term_to_linked_terms)   
    #term_to_linkedsup_terms = term_to_linked_superterms(efo_og, term_to_linked_terms)
    term_to_linkedsup_terms.update(term_to_linked_superterms(efo_cell_og, term_to_linked_terms))
    term_to_linkedsup_terms.update(term_to_linked_superterms(efo_cellline_og, term_to_linked_terms))
    term_to_linkedsup_terms.update(term_to_linked_superterms(cvcl_og, term_to_linked_terms))
    ...
    with open(config.ref_path / "term_to_superterm_linked_terms.json", "w") as f:
        f.write(jsonio.dumps(term_to_linkedsup_terms))

def term_to_linked_superterms(og, term_to_linked_terms):
